...
class MatchData:
    ...
    def __init__(self, match_vec, match_pairs_vec, best_angle_histogram, best_length_histogram):
        """
        :param match_vec: array of n features of that were found a match
        :type match_vec: ndarray (nx1)

        :param match_pairs_vec: array of match pairs (point in image 1 vs point in image 2)
        :type match_pairs_vec: ndarray (nx2)
        ...
        self.good = match_vec
        self.good_match_pair_vec = match_pairs_vec
        self.angle_histogram = best_angle_histogram
        self.length_histogram = best_length_histogram

    def get_data(self):
        return [self.angle_histogram, self.length_histogram, self.good, self.good_match_pair_vec]
    ...
    def filter_data_by_histogram(self):
        ...
        filtered_match_pairs = [None] * n_final_matches
        ...
        for match_index, match_object in enumerate(self.good):
            ...
            if self.angle_histogram.outliers[match_index] == False and \
               self.length_histogram.outliers[match_index] == False:
                ...
                filtered_match_pairs[i] = self.good_match_pair_vec[match_index]
                i += 1
